...
def get_network(network_name, architecture, experiment):
    ...
    if architecture == "ring":
        ...
        # Self-loop: local model is always available
        for worker_id in range(11):
            network.add_edge(
                worker_id,
                worker_id
        ...
        return network
    elif architecture == "sfl":
        # SFL still has the same 11 workers.
        # The server/global model is handled in communication.py,
        # so no GML communication graph is required here.
        network = nx.Graph()
        network.add_nodes_from(
            range(11)
        )
...
def get_iterator(name, path, device, batch_size):
    ...
    elif name in ("driving_gazebo", "driving_carla"):
...
def loggs_to_json(loggs_dir_path):
    ...
    :param loggs_dir_path: path to logs folder
    """
    ...
    for dname in os.listdir(loggs_dir_path):
        ...
        for tag in tags:
            ...
            for event in ea.Scalars(tag):
                ...
                tag_steps.append(event.step)

            # Dynamically support ns-3 metrics
            if tag not in all_results:
                all_results[tag] = {}

            all_results[tag][dname] = tag_values

            if len(tag_steps) > len(steps):
                steps = tag_steps
    ...
    json_path = os.path.join(
        ...
        "json",
        "{}.json".format(
            os.path.split(loggs_dir_path)[1]
        )
    )
